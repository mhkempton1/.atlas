    ...
    return (
        <div className="h-full flex flex-col space-y-6 animate-slide-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full overflow-hidden">
                        ...
                        <Section title="Priority Actions">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {priorityItems.map((item, idx) => (
                                    <div key={item.id || idx} className="bg-slate-800/40 border border-purple-500/30 rounded-xl p-4 relative overflow-hidden group">
                        ...
                        {upcomingItems.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {upcomingItems.map((item, idx) => (
                                    <div key={item.id || idx} className="flex items-center justify-between p-4 bg-slate-800/30 border border-white/5 rounded-lg hover:border-slate-500 transition-colors">
                                        <div>
                ...
                <div className="bg-slate-800/30 border border-white/5 rounded-xl ml-1 flex flex-col h-full overflow-hidden">
                    <div className="p-4 border-b border-white/5 flex justify-between items-center">
                        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Personnel Directory</h3>
                        <span className="text-[10px] bg-slate-800 text-gray-400 px-2 py-1 rounded">3 Online</span>
                    </div>

                    <div className="p-4">
                        <input className="w-full bg-slate-900 border border-white/10 rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-purple-500/50" placeholder="Search directory..." />
                    </div>
